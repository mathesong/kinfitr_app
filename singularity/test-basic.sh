...
echo "=== kinfitr Singularity Integration Test ==="
echo
...
if [ -f "kinfitr.def" ]; then
    # Basic syntax checks
    if grep -q "^Bootstrap: docker" kinfitr.def; then
    ...
    if grep -q "^From: rocker/shiny-verse" kinfitr.def; then
    ...
    if grep -q "%runscript" kinfitr.def; then
