...
This report summarises the data subsetting and individual TACs file creation step of the kinfitr analysis pipeline.
...
if (!is.null(config$Subsetting)) {
  ...
  subsetting_df <- tibble(
    Parameter = names(subsetting),
    Value = map_chr(subsetting, ~ if(.x == "" || is.null(.x)) "All" else as.character(.x))
  )
  kable(subsetting_df)
} else {
...
# Function to detect TAC radioactivity units from TAC JSON sidecar files
detect_tac_radioactivity_units <- function(tac_filename, analysis_folder) {
  # Convert TSV filename to JSON filename for TAC sidecar file
  json_filename <- str_replace(tac_filename, "\\.tsv$", ".json")
  json_path <- file.path(analysis_folder, json_filename)
  ...
  if (file.exists(json_path)) {
    ...
    tryCatch({
      ...
      # If JSON reading fails, return Bq
      return("Bq")
    })
  }
...
# Apply TAC unit conversion to standardize to kBq
tacdata <- tacdata %>%
  group_by(pet) %>%
  mutate(
    detected_units = first(detect_tac_radioactivity_units(pet, analysis_folder)),
    TAC = kinfitr::unit_convert(TAC, from_units = detected_units, to_units = "kBq")
  ) %>%
  select(-detected_units) %>%
  ungroup()
```
