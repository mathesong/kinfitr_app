...
if (!is.null(config$FitDelay)) {
  ...
  method_display <- switch(delay_config$model %||% "zero",
    "zero" = "Set to zero (no delay fitting)",
...
delay_method <- config$FitDelay$model %||% "zero"

do_zero_delay <- delay_method == "zero"
fit_delay <- !do_zero_delay
# do_lin2tcm_singletac <- delay_method == "lin2tcm_singletac"
...
```{r}
### NOTE: Remove this when the data are fixed. TAC units are wrong:
### They claim to be kBq, but they are actually Bq
tac_data <- tac_data %>% 
  mutate(TAC = TAC/1000)
```
...
```{r, eval=do_zero_delay, echo=do_zero_delay, results='asis'}
str_glue("## Zero Delay (No Fitting)")
```

```{r, eval=do_zero_delay, echo=do_zero_delay}
# Zero delay implementation
# All delays are set to zero - no estimation required

delay_res <- model_data %>% 
  mutate(inpshift = 0) %>% 
  select(-tacs, -input) %>% 
  ungroup()
```
...
```{r, eval=do_zero_delay, echo=do_zero_delay, results='asis'}
str_glue("## Saving Outcomes")
```

```{r, eval=do_zero_delay, echo=do_zero_delay}
delay_res <- delay_res %>% 
  mutate(filename = paste0(output_stem, "_auxkinpar.json"))

save_inpshift <- function(inpshift, method, filename) {
  out <- list(
    blood_timeshift = list(
      Value = inpshift,
      Method = method
    ))
  json_output <- jsonlite::toJSON(out, pretty = TRUE, auto_unbox = TRUE)
  writeLines(json_output, filename)
}

pwalk(list(delay_res$inpshift, 
           method_display, 
           paste0(analysis_folder, "/", delay_res$filename)),
      save_inpshift)
```

```{r, eval=do_zero_delay, echo=do_zero_delay, results='asis'}
str_glue("# Session Information")
```
