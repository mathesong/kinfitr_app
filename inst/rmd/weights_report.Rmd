...
```{r}
analysis_folder <- "/home/granville/Repositories/OpenNeuro/ds004869/derivatives/kinfitr/Primary_Analysis/"
```
...
# Use analysis_folder (from params or debugging section above)
...
This report summarises the weights calculation step of the kinfitr analysis pipeline.
...
<!--   unnest(tacs) %>% -->
<!--   rename(pet = basename) %>% -->
<!--   mutate(pet = str_remove(pet, "_desc-combinedregions_tacs.tsv")) %>% -->
...
<!-- region_type <- config$Weights$region_type %||% "mean_combined" -->
<!-- region_name <- config$Weights$region %||% "" -->
<!-- external_tacs <- config$Weights$external_tacs %||% "" -->
<!-- radioisotope <- config$Weights$radioisotope %||% "C11" -->
<!-- halflife_raw <- config$Weights$halflife %||% "" -->
<!-- method <- as.numeric(config$Weights$method %||% "2") -->
<!-- custom_formula_raw <- config$Weights$custom_formula %||% "" -->
<!-- minweight <- as.numeric(config$Weights$minweight %||% 0.25) -->
...
<!-- cat("Region type:", region_type, "\n") -->
<!-- cat("Method:", method, "\n") -->
<!-- cat("Radioisotope:", radioisotope, "\n") -->
<!-- cat("Half-life:", if(is.null(halflife)) "Default" else paste(halflife, "min"), "\n") -->
<!-- cat("Minimum weight:", minweight, "\n") -->
<!-- ``` -->
...
mean_tacdata <- weights_tacdata %>%
  ...
  # Re-extract attributes columns for matching
  mutate(attributes = map(pet, ~kinfitr:::bids_filename_attributes(paste0(.x, "_tacs")))) %>% 
  unnest(attributes) %>% 
  select(-measurement)
...
selected_desc <- config$Weights$external_tacs
...
kinfitr_dir <- dirname(analysis_folder)
...
if (file.exists(combined_regions_file)) {
  ...
  # Filter for the selected segmentation (desc) and extract seg_meanTAC
  if ("seg_meanTAC" %in% colnames(combined_regions_data) && "desc" %in% colnames(combined_regions_data)) {
    segmentation_data <- combined_regions_data %>%
      filter(desc == selected_desc) %>%
    ...
    if (nrow(segmentation_data) > 0) {
      cat("Successfully loaded segmentation mean TACs for desc:", selected_desc, "\n")
      # cat("Total measurements found:", length(unique(weights_tacdata$pet)), "\n")
    } else {
      stop("No data found for segmentation desc: ", selected_desc)
    }
  } else {
    ...
    if (!"desc" %in% colnames(combined_regions_data)) {
      stop("desc column not found in combined regions file")
    }
  }
} else {
...
str_glue("Now we normalise and check for minweight issues")
```
