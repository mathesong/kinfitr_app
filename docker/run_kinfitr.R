...
# Docker entry script for kinfitr apps
...
option_list <- list(
  ...
  make_option(c("--kinfitr_output_foldername"), type="character", default="kinfitr", 
              help="Name for kinfitr output folder within derivatives [default: kinfitr]"),
...
opt_parser <- OptionParser(option_list=option_list, 
                          description="Docker entry point for kinfitr apps")
...
cat("=== kinfitr Docker Container ===\n")
cat("Function:", opt$func, "\n")
cat("Mode:", opt$mode, "\n")
if (!is.null(opt$step)) {
  cat("Step:", opt$step, "\n")
}
cat("kinfitr output folder:", opt$kinfitr_output_foldername, "\n")
...
if (opt$mode == "interactive") {
  ...
  kinfitr_dir <- file.path(derivatives_path, "kinfitr")
  analysis_folder <- file.path(kinfitr_dir, opt$analysis_foldername)
