...
test_that("create_kinfitr_regions_files works with valid input", {
  ...
  kinfitr_regions_file <- file.path(bids_dir, "code", "kinfitr", "kinfitr_regions.tsv")
  
  result <- create_kinfitr_regions_files(kinfitr_regions_file, derivatives_folder)
...
test_that("combine_regions_from_files processes data correctly", {
  ...
  tacs_files <- list.files(
    file.path(derivatives_folder, "kinfitr", "sub-01", "ses-01"),
    pattern = "*_tacs.tsv", 
    full.names = TRUE
  )
...
test_that("create_kinfitr_combined_tacs integrates all components", {
  ...
  bids_dir <- create_test_bids_structure(temp_dir, n_subjects = 2, n_sessions = 1)
  derivatives_folder <- file.path(bids_dir, "derivatives")
  output_dir <- file.path(derivatives_folder, "kinfitr")
  
  kinfitr_regions_file <- file.path(bids_dir, "code", "kinfitr", "kinfitr_regions.tsv")
  
  result <- create_kinfitr_combined_tacs(
    kinfitr_regions_file,
    derivatives_folder,
    output_dir,
    bids_dir
  )
...
test_that("process_all_kinfitr_regions handles full workflow", {
  ...
  bids_dir <- create_test_bids_structure(temp_dir, n_subjects = 2, n_sessions = 1)
  derivatives_folder <- file.path(bids_dir, "derivatives")
  output_folder <- file.path(derivatives_folder, "kinfitr")
  
  kinfitr_regions_file <- file.path(bids_dir, "code", "kinfitr", "kinfitr_regions.tsv")
  
  result <- process_all_kinfitr_regions(kinfitr_regions_file, derivatives_folder, output_folder)
