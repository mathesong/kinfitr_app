...
test_that("generate_step_report creates step-specific reports", {
  ...
  analysis_folder <- file.path(bids_dir, "derivatives", "kinfitr", "Primary_Analysis")
  ...
  config_file <- file.path(analysis_folder, "desc-kinfitroptions_config.json")
...
test_that("report generation validates report template existence", {
  ...
  template_1tcm <- system.file("rmd", "1tcm_report.Rmd", package = "kinfitrapp")
  template_2tcm <- system.file("rmd", "2tcm_report.Rmd", package = "kinfitrapp") 
  template_weights <- system.file("rmd", "weights_report.Rmd", package = "kinfitrapp")
