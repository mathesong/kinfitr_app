# Test setup helpers and utilities for kinfitrapp tests
...
create_test_bids_structure <- function(base_dir = tempdir(), n_subjects = 2, n_sessions = 1) {
  ...
  dir.create(file.path(bids_dir, "derivatives", "kinfitr"), showWarnings = FALSE)
  dir.create(file.path(bids_dir, "code", "kinfitr"), recursive = TRUE, showWarnings = FALSE)
  ...
  for (sub in 1:n_subjects) {
    ...
    for (ses in 1:n_sessions) {
      ...
      deriv_sub_ses <- file.path(bids_dir, "derivatives", "kinfitr", sub_id, ses_id)
  ...
  # Create kinfitr regions file
  create_test_kinfitr_regions(bids_dir)
...
create_test_tacs_file <- function(output_dir, sub_id, ses_id) {
  ...
  test_data_path <- system.file("extdata", "test_tac_data.csv", package = "kinfitrapp")
  if (!file.exists(test_data_path)) {
    # Fallback to data directory
    test_data_path <- file.path(find.package("kinfitrapp"), "..", "data", "test_tac_data.csv")
  }
...
#' Create test kinfitr regions file
create_test_kinfitr_regions <- function(bids_dir) {
  ...
  regions_file <- file.path(bids_dir, "code", "kinfitr", "kinfitr_regions.tsv")
